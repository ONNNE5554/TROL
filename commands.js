...
export default async function (bot, interaction, type = "slash", settings) {
  ...
  let stop_command_names = await settings.has("stop_command_names") ? await settings.get("stop_command_names") : ["stop", "-وقف"]
  ...
  if (interaction.type == 2 && roulette_command_names.map(e => e.toLowerCase()).includes(interaction.data.name.toLowerCase())) {
    ...
    if (roulette_games.has(interaction.guildID)) return await interaction.createMessage({
      "flags": 64,
      "content": ":x: | قاعدين يلعبون يالفاهي    "
    })
  }
    ...
    collecter_buttons.on('collect', async i => {
      ...
      if (data[0] == "leave") {
        ...
        await i.createMessage({ flags: 64, content: `✅ | طردك من اللعبه` });
        ...
        let roulette_data = roulette_games.get(i.guildID)
        if (roulette_data.players.length >= 0) return await i.createMessage({ flags: 64, content: "اكتملو اللعيبه  " })
        if (roulette_data.players[0] && roulette_data.players.some(player => player.id == i.member.id)) return await i.createMessage({ flags: 64, content: "انت مسجل اقعد ياتطرد ياتنطرد             " });
        await i.deferUpdate();
        if (data[1] == "random") {
    ...
    collecter_buttons.on("end", async (interactions, r) => {
      ...
      if (roulette_games.has(interaction.guildID) && !roulette_games.get(interaction.guildID).players[2]) {
        ...
        await interaction.channel.createMessage("✅ | تم الانتهاء من التسجيل. راح نبداء الحين     ...");
        await startRoundRoulette(bot, interaction, roulette_games, id)
      } else if (!roulette_games.has(interaction.guildID)) {
        await interaction.channel.createMessage(":x: | تم إيقاف الروليت بواسطة المسؤولين");
      }
    })
})
}